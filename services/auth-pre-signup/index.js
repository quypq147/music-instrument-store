...
// services/auth-pre-signup/index.ts
var index_exports = {};
__export(index_exports, {
  AUTO_LINKED_MARKER: () => AUTO_LINKED_MARKER,
  handler: () => handler
});
...
var cognitoClient = import_aws_xray_sdk_core.default.captureAWSv3Client(
  new import_client_cognito_identity_provider.CognitoIdentityProviderClient({})
);
...
var AUTO_LINKED_MARKER = "AUTO_LINKED_RETRY";
var generateRandomPassword = () => `${(0, import_node_crypto.randomBytes)(24).toString("base64url")}aZ2!`;
var capitalize = (value) => value.charAt(0).toUpperCase() + value.slice(1).toLowerCase();
var parseFederatedUserName = (userName) => {
...
var findUsersByEmail = async (userPoolId, email) => {
...
var linkProviderToUser = async (userPoolId, destinationUsername, providerName, providerUserId) => {
...
var createProfileRecord = async (userAttributes) => {
...
var handler = async (event) => {
  ...
  const nativeUser = existingUsers.find(
    (user) => user.UserStatus !== "EXTERNAL_PROVIDER" && user.UserStatus !== "UNCONFIRMED"
  );
  try {
    if (nativeUser?.Username) {
      ...
      const created = await cognitoClient.send(
        new import_client_cognito_identity_provider.AdminCreateUserCommand({
          ...
          Username: (0, import_node_crypto.randomUUID)(),
          MessageAction: "SUPPRESS",
          UserAttributes: [
            { Name: "email", Value: email },
            { Name: "email_verified", Value: "true" },
            ...event.request.userAttributes.name ? [{ Name: "name", Value: event.request.userAttributes.name }] : []
          ]
        })
      );
