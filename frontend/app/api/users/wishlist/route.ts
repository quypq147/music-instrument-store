...
export async function GET(req: NextRequest) {
  try {
    ...
    const res = await fetch(`${apiGatewayUrl}/users/wishlist`, {
      method: "GET",
      headers,
    });
    ...
    console.error("Proxy GET /api/users/wishlist error:", error);
    return NextResponse.json(
...
export async function POST(req: NextRequest) {
  try {
    ...
    console.error("Proxy POST /api/users/wishlist error:", error);
    return NextResponse.json(
      { error: "Internal Server Error" },
      { status: 500 }
    );
