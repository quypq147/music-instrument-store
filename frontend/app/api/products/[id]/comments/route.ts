...
export async function GET(req: NextRequest, { params }: Params) {
  try {
    ...
    const targetUrl = `${apiGatewayUrl}/products/${encodeURIComponent(id)}/comments`;
    ...
    console.error("Proxy GET /api/products/[id]/comments error:", error);
    return NextResponse.json(
...
export async function POST(req: NextRequest, { params }: Params) {
  try {
    ...
    console.error("Proxy POST /api/products/[id]/comments error:", error);
    return NextResponse.json(
      { error: "Internal Server Error" },
      { status: 500 }
    );
